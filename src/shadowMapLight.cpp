...
void ShadowMapLight::setup( int shadowMapSize, float fov, float _near, float _far ) {

    // make a projection matrix that we'll use to render a scene from our light's viewpoint
    m_projectionMatrix.makePerspectiveMatrix(fov, shadowMapSize/shadowMapSize, _near, _far);
    ...
    m_linearDepthScalar = 1.0 / (_far - _near); // this helps us remap depth values to be linear
