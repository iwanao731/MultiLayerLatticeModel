...
void testApp::setup(){
	...
	// Boomer
	//obj.Load("data/model/Boomer/64/point.obj");
	//surface.Load("data/model/Boomer/64/skin.obj");
	//surface2 = surface;

	// Bloat(16)
	//obj.Load("data/model/bloat/16/point.obj");
	//surface.Load("data/model/bloat/16/Bloat_skin.obj");
	//surface2 = surface;

	// Bloat(32)
	//obj.Load("data/model/bloat/32/point.obj");
	//surface.Load("data/model/bloat/32/Bloat_skin.obj");
	//surface2 = surface;

	// Bloat(64)
	//obj.Load("data/model/bloat/64/point32.obj");
	//surface.Load("data/model/bloat/64/skin32.obj");
	//surface2 = surface;

	// Smurf
	//obj.Load("data/model/Smurf/64/point.obj");
	//surface.Load("data/model/Smurf/64/skin.obj");
	//surface2 = surface;

	// Hot Girl
	//obj.Load("data/model/Hot_Girl/Hot_Girl_01_scale_point.obj");
	//surface.Load("data/model/Hot_Girl/Hot_Girl_skin.obj");
	//surface2 = surface;
	...
	// BVH
	// Rabbit Dancing
	//bvh.load("model/Rabbit/naturaldance_take_001_copy.bvh");
	//origin.load("model/Rabbit/naturaldance_take_001_copy.bvh");

	bvhFileName = "model/Rabbit/Naturaldance_Take_024.bvh";
	//bvhFileName = "data/model/bloat/32/Bloat_bone_32_9.bvh";
	//bvhFileName = "model/Rabbit/ArmBend3.bvh";
	//bvhFileName = "data/model/Smurf/64/jump_jump2.bvh";
	//bvhFileName = "data/model/bloat/16/Bloat_bone_32_9_scaled.bvh";

	bvh.load(bvhFileName);
	origin.load(bvhFileName);

	// Rabbit Armbend
	//bvh.load("model/Rabbit/ArmBend2.bvh");
	//origin.load("model/Rabbit/ArmBend2.bvh");

	// bloat
	// 16
	//bvh.load("data/model/bloat/bloat/16/bloat.bvh");
	//origin.load("data/model/bloat//16/bloat.bvh.bvh");

	// 32
	//bvh.load("data/model/bloat/32/Bloat_bone_32_4.bvh");
	//origin.load("data/model/bloat//32/Bloat_bone_32_4.bvh");

	// Boomer
	//bvh.load("data/model/Boomer/64/bone.bvh");
	//origin.load("data/model/Boomer/64/bone.bvh");

	// Smurf
	//bvh.load("data/model/Smurf/64/dance.bvh");
	//origin.load("data/model/Smurf/64/dance.bvh");
	
	//bvh.load("data/model/Smurf/64/jump.bvh");			
	//origin.load("data/model/Smurf/64/jump.bvh");
	//bvh.load("data/model/Smurf/64/jump_jump2.bvh");
	//origin.load("data/model/Smurf/64/jump_jump2.bvh");

	// Hot Girl
	//bvh.load("data/model/Hot_Girl/export.bvh");
	//origin.load("data/model/Hot_Girl/export.bvh");
	...
	// MUSIC
	//sound.loadSound("music/timemachine.wav"); //サウンドファイルの読込み
	//sound.setLoop(true); //ループ再生をONに
	//sound.play(); //サウンド再生開始
...
//--------------------------------------------------------------
void testApp::setManhattanWeight()
{

}

//--------------------------------------------------------------
void testApp::guiSetup1()
{
	screen = false;
	_condition = false;
	volume = 0.0;
...
void testApp::update()
{
	if(_condition){
		...
		//{
		//	char imgname[256];
		//	//sprintf_s(imgname, 256, "C:/Users/n.iwamoto/Desktop/rabbit/%d.jpg", _frame );
		//	//sprintf_s(imgname, 256, "C:/Users/iwanao/Desktop/rabbit/%d.jpg", _frame );
		//	sprintf_s(imgname, 256, "J:/project/rabbit_dance/JiggingMesh/img/%d.jpg", _frame );

		//	// Capture Screen
		//	grabbedImage.grabScreen(0, 0, ofGetWidth(), ofGetHeight());

		//	// Save the chaptured image data
		//	grabbedImage.saveImage(imgname);
		//}

		// Save
		//char filename[256];
		//sprintf_s(filename, 256, "c:/users/n.iwamoto/desktop/rabbit/%d.obj", _frame );
		//sprintf_s(filename, 256, "j:/project/rabbit_dance/jiggingmesh/demo2/model/boomer/%d.obj", _frame );
		//surface2.Save(filename);

		//sumVolume += abs(LSM.SendCurrentVolume() - LSM.SendInitialVolume());
...
//	glEnable( GL_DEPTH_TEST );
    ...
    //m_shadowLight.lookAt( ofVec3f(0.0, 450.0, 0.0) );
	...
	//		ofLine(nearestVertex, ofVec3f(mouseX, mouseY));
		}
	}
	glEnable(GL_DEPTH_TEST);	
...
void testApp::drawMain()
{
   ...
   glCullFace(GL_BACK);
	...
	if(_floor){
		...
		//ofTranslate(0.0, -1.6, 0.0); // rabbit rotation
		ofTranslate(0.0, 0.1, 0.0); // rabbit
		//ofTranslate(0.0, -1.0, 0.0); // Smurf
		//ofTranslate(0.0, -1.4, 0.0); // Bloat 32
		//ofTranslate(0.0, -1.1, 0.0); // Bloat 16
		ofBox(0,0,0,2.0f);
		ofPopMatrix();
	}
...
//	if(_condition){
		//string file = "L:/Research/Projects/SkeletonBasedPhysics/CGF/demo/Smurf/20150129/obj/" +ofToString(_frame) + ".obj";
		//surface2.Save(file);
		//cout << _frame << endl;
//	}

	LSM.CalcVolume(&surface2);
	//LSM.RenderForceDirection(&bvh, _condition, renderExternalForce);
	//ofSetColor(51.0, 102.0, 204.0, _alpha);
	//
	//if(_wire)
		//LSM.Render2(boneNum, _alpha);
	...
	//LSM.RenderParticle(boneNum, _renderParticle);
	...
	//ofRotate(90, 0, 1, 0);
	//ofRotate(90, 1, 0, 0);
	//ofPoint p = bvh.getJoint(0)->getPosition();
	
	//ofSetColor(100, 50);
	//p.y -= 300;
	//ofCircle(-p.z, p.y, -220, 80);

	////p.y -= 130;
	////ofCircle(-p.z, p.y, 75, 130);
	//ofPopMatrix();
	
	//ofPushMatrix();
	//ofRotate(180, 0,1,0);
	//ofSetColor(255,255,0);
	//ofSphere(ballPos, radius);
	//ofPopMatrix();
}
//--------------------------------------------------------------
void testApp::exit()
...
void testApp::mouseDragged(int x, int y, int button){
	...
	//ofSetColor(255, 0, 0);
	//ofCircle(ofPoint(mouseX, mouseY), 1.0f);
...
void testApp::setupLights() {
	...
	//m_shadowLight.setPosition( -300.0, 100.0, 0.0 );	//rabbit rotation
	//m_shadowLight.lookAt( ofVec3f(0.0, 00.0, 0.0) );

	//m_shadowLight.setPosition( 600.0, 350.0, 0.0 );	//Bloat
	//m_shadowLight.lookAt( ofVec3f(0.0, 250.0, 0.0) );
    
	//m_shadowLight.setPosition( -300.0, 450.0, 600.0 );	//Smurf
	//m_shadowLight.lookAt( ofVec3f(0.0, 350.0, 200.0) );

	ofSetGlobalAmbientColor( ofFloatColor( 0.05f, 0.05f, 0.05f ) );
}

//--------------------------------------------------------------
void testApp::RenderIndex(int i){
...
//--------------------------------------------------------------
VERTEX3D testApp::ErrorToRGB(float err, float errMax, float errMin)
{
	int r, g, b;
...
//--------------------------------------------------------------
float testApp::SkinWeight(ofVec3f P0, ofVec3f P1, ofVec3f P2, int c)
{
	float t, d;
	ofVec3f P;
